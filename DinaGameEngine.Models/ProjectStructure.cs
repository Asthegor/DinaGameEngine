namespace DinaGameEngine.Models
{
    public static class ProjectStructure
    {
        public const string RecentProjectsFileName = "recentProjects.json";
        public const string ProjectFileName = "dina.project.json";
        public const string IconFileName = "Icon.ico";

        public const string FontsProject = "Fonts/Fonts.csproj";
        public const string AudioProject = "Audio/Audio.csproj";
        public const string AssetsProject = "Assets/Assets.csproj";
        public const string CoreProject = "Core/Core.csproj";
        public const string ScenesProject = "Scenes/Scenes.csproj";
        
        public const string LogFileName = "log.txt";

        public const string SectionStatesFileName = "sectionProjects.json";
    }
}
