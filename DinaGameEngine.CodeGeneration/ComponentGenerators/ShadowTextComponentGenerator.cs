    ...
    public class ShadowTextComponentGenerator : ComponentGenerator, IComponentGenerator
    {
        ...
        protected override void GenerateLoad(SectionParser sectionParser, ComponentModel component, int level)
        {
            ...
            if (!string.IsNullOrEmpty(visibleValue))
                sectionParser.InsertIntoZone("COMPONENT_LOAD", [CodeBuilder.AddLine($"{GetFieldName(component)}.Visible = {visibleValue.ToLower()};", level)]);

            var horizontalAlignment = ComponentPropertyHelper.GetStringProperty(component, "HorizontalAlignment");
            var verticalAlignment = ComponentPropertyHelper.GetStringProperty(component, "VerticalAlignment");
            if (!string.IsNullOrEmpty(horizontalAlignment) || !string.IsNullOrEmpty(verticalAlignment))
            {
                ...
                sectionParser.InsertIntoZone("COMPONENT_LOAD", [CodeBuilder.AddLine($"{GetFieldName(component)}.SetAlignments(HorizontalAlignment.{h}, VerticalAlignment.{v});", level)]);
            }
            ...
            foreach (var property in component.Properties)
            {
                if (!excludedKeys.Contains(property.Key))
                    sectionParser.InsertIntoZone("COMPONENT_LOAD", [CodeBuilder.AddLine($"{GetFieldName(component)}.{property.Key} = {property.Value};", level)]);
            }
        }
