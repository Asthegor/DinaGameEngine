...
namespace __RootNamespace__.Scenes
{
    public class OptionsScene(SceneManager sceneManager) : Scene(sceneManager)
    {
        ...
        public override void Load()
        {
            ...
            _categoryFont = fontManager.Load(FontKeys.Default);
            _labelFont = fontManager.Load(FontKeys.Default);
            _optionFont = fontManager.Load(FontKeys.Default);
            ...
            var titleFont = fontManager.Load(FontKeys.Default);
            var title = new ShadowText(titleFont, "OPTIONS_TITLE", PaletteColors.MainMenu_Title, PaletteColors.MainMenu_Title_Shadow, offsetTitle, new Vector2(0, ScreenDimensions.Y / 16), HorizontalAlignment.Center);
